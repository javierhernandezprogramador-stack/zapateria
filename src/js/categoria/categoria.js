...
function eventos() {
    ...
    $('#btnGuardar-categoria').click(() => {
        if ($('#btnGuardar-categoria').val() == 'Agregar') {
            almacenarCategoria();
        } else if ($('#btnGuardar-categoria').val() == 'Modificar') {
            modificarCategoria();
        }
...
function almacenarCategoria() {
    let url = '../../api/requestCategoria.php';
    let categoria = {
        nombre: $('#nombreCategoria').val()
    };

    if (!validarCategoria(categoria)) return;
...
function obtenerTodasCategorias() {
    console.log("acceso al fetch de categorias");
...
function eliminarCategoria(code) {
    let url = '../../api/requestCategoria.php';
    ...
    fetch(url, {
        ...
        if (data.resultado == 1) {
            ...
            setTimeout(() => {
                window.location.href = '../../modulos/categorias/';
            }, 500);
        }
    }).catch(error => {
        toastr.error("No se puede eliminar esta categoria");
        console.log(error);
    });
}
...
function levantarModal(categoria) {
    $('#nombreCategoria').val(categoria.nombre);
    $('#idCategoria').val(categoria.id);

    llamarFormCategoria("Modificar", "Modificar Categoria");
}
