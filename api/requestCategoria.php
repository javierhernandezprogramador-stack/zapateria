...
function validarOperacion($operacion, $obj, $id)
{
    ...
    require_once ROOT_PATH . '/models/DaoCategoria.php';

    $dao = new DaoCategoria();

    switch ($operacion) {
        case 'registrar':
            echo json_encode($dao->save($obj->categoria));
            break;
        case 'modificar':
            echo json_encode($dao->update($obj->categoria, $id));
            break;
        case 'eliminar':
        ...
        case 'obtenerCantidad':
            echo json_encode($dao->obtenerCantidadCategoria());
            break;
        case 'listarTodos':
