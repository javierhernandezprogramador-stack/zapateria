...
class PDF extends FPDF {
    ...
    function PremiumTable($data) {
        ...
        $widths = [30, 120, 60, 60]; // Anchos de las columnas
        ...
        foreach ($data as $row) {
            ...
            $this->Cell($widths[1], 10, utf8_decode($row['proveedor']), "LR", 0, "L", $fill); // Nombre
            $this->Cell($widths[2], 10, utf8_decode($row['fecha_compra']), "LR", 0, "C", $fill); // Cantidad
            $this->Cell($widths[3], 10, utf8_decode("$" . $row['total']), "LR", 0, "C", $fill); // Estado
            $this->Ln();
...
include "../models/DaoCompra.php"; // Asegúrate de que esta clase existe
...
$pdf->AddPage();
$pdf->DocumentTitle();
$pdf->PremiumTable(DaoCompra::listarCompras()); // Llamada al método para obtener los datos
$pdf->Output();
?>
