...
class PDF extends FPDF
{
    ...
    private $title = "Flujo de efectivo";
    ...
    function PremiumTable($data)
    {
        ...
        $header = ["Ingresos", "Egresos", "Diferencia"];
        $widths = [60, 60, 60]; // Anchos de las columnas
        ...
        foreach ($data as $row) {
            ...
            $this->Cell($widths[0], 10, utf8_decode("$" . number_format($row['total_venta'], 2)), "LR", 0, "C", $fill); // ID
            $this->Cell($widths[1], 10, utf8_decode("$" . number_format($row['total_compra'], 2)), "LR", 0, "C", $fill); // Nombre

            $diferencia = $row['total_venta'] - $row['total_compra'];

            if ($diferencia < 0) {
                $this->SetTextColor(255, 0, 0);
                $this->Cell($widths[2], 10, utf8_decode("$" . number_format(abs($diferencia), 2)), "LR", 0, "C", $fill); // Cantidad
            } else {
                $this->SetTextColor(0, 143, 57);
                $this->Cell($widths[2], 10, utf8_decode("$" . number_format(abs($diferencia), 2)), "LR", 0, "C", $fill); // Cantidad
...
$pdf->AddPage();
$pdf->DocumentTitle();
$pdf->PremiumTable(DaoCompra::flujoEfectivo()); // Llamada al método para obtener los datos
$pdf->Output();
